...
const parseFourWings = (arrayBuffer, minFrame, maxFrame) => {
  ...
  const timeseries = framesInCell
    ...
    : [];
  console.log(
    `Timeseries from ${getDate(minFrame)} to ${getDate(maxFrame)}`,
    timeseries
  );

  const aggregatedValues = framesInCell
    ? framesInCell.map((cell) =>
        cell.reduce((prev, curr) => prev + curr.value, 0)
      )
    : 0;
  console.log(
  ...
  for (let i = FEATURE_CELLS_START_INDEX; i < intArray.length; i++) {
    ...
    if (indexInCell === CELL_NUM_INDEX) {
      ...
      endIndex =
        startIndex +
        CELL_VALUES_START_INDEX +
        (endFrame - startFrame + 1) * sublayerCount;
    }
    indexInCell++;
    if (i === endIndex - 1) {
      indexInCell = 0;
      ...
      const values = intArray.slice(
        startIndex + CELL_VALUES_START_INDEX,
        endIndex
      );
